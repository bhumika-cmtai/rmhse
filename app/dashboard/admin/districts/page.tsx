...
// MODIFIED: Initial form state now defaults the role to 'DIST'
...
export default function DivUsers() {
  ...
  // MODIFIED: The role is now hardcoded to 'DIST' in the fetchUsers call.
  useEffect(() => {
    ...
    const params: any = {
      ...
      role: 'DIST', // Always fetch users with the 'DIST' role
      status: status,
    };
  ...
  const openAddModal = () => {
    setEditUser(null);
    setForm(initialFormState); // This correctly sets the default role to 'DIST'
    setIsModalOpen(true);
  };
  ...
  const refreshData = () => {
    // MODIFIED: Ensure refresh also fetches only 'DIST' role
  ...
  return (
    <div className="w-full mx-auto mt-2">
      <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-8">
        ...
        <h1 className="text-4xl font-bold shrink-0">DIST User List</h1>
        <div className="flex flex-wrap justify-end gap-2 w-full">
          ...
          {/* <Button size="sm" className="gap-1" onClick={openAddModal}>
            <Plus className="w-4 h-4"/> Add DIST User
          </Button> */}
        </div>
      </div>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="sm:max-w-[800px]">
          <DialogHeader>
            <DialogTitle>
              {editUser ? 'Edit DIST User' : 'Add New DIST User'}
            </DialogTitle>
            <DialogDescription>
          ...
          <form  className="space-y-4 mt-2 max-h-[70vh] overflow-y-auto pr-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {/* MODIFIED: Role selection is now disabled as it's fixed to 'DIST' */}
              <div className="space-y-2">
