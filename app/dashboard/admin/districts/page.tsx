...
// --- Filter for District Users ---
// This new page will only work with users who have the 'dist' role.
const districtUsers = staticUsers.filter(user => user.role === 'dist');

// The initial form state will default the role to 'dist' and make it read-only for this page.
...
export default function DistrictsPage() {
  // State is initialized with only the district users.
  const [allDistricts, setAllDistricts] = useState<User[]>(districtUsers);
  const [displayedDistricts, setDisplayedDistricts] = useState<User[]>([]);
  ...
  useEffect(() => {
    ...
    const filteredUsers = allDistricts.filter(user => {
    ...
    setDisplayedDistricts(paginatedUsers);
    
    setLoading(false);
  }, [allDistricts, debouncedSearch, status, page]);
  ...
  const openEditModal = (user: User) => {
    ...
    setForm({
      ...
      role: 'dist', // Role is fixed to 'dist'
  ...
  const handleFormSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (editUser && editUser._id) {
        ...
        const updatedDistricts = allDistricts.map(u => u._id === editUser._id ? updatedUser : u);
        ...
        setAllDistricts(prevUsers => [newUser, ...prevUsers]);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      setAllDistricts(prev => prev.filter(u => u._id !== userToDelete._id));
      toast.success(`District "${userToDelete.name}" deleted successfully.`);
      setIsDeleteModalOpen(false);
      setUserToDelete(null);
      if (displayedDistricts.length === 1 && page > 1) setPage(page - 1);
    } finally {
  ...
  return (
    <div className="w-full mx-auto mt-2">
      <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-8">
        <h1 className="text-4xl font-bold shrink-0">District List</h1>
        <div className="flex flex-wrap justify-end gap-2 w-full">
          ...
          <Button size="sm" className="gap-1" onClick={openAddModal}><Plus className="w-4 h-4"/> Add District</Button>
        </div>
      </div>
        ...
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <Table>
              ...
              <TableBody>
                {loading ? (
                  <TableRow><TableCell colSpan={8} className="text-center py-8"><div className="flex justify-center items-center gap-2"><Loader2 className="h-6 w-6 animate-spin" /><span>Loading districts...</span></div></TableCell></TableRow>
                ) : displayedDistricts.length === 0 ? (
                  <TableRow><TableCell colSpan={8} className="text-center py-8">{debouncedSearch || status !== "all" ? "No districts found for the current filters." : "No districts found."}</TableCell></TableRow>
                ) : (
                  displayedDistricts.map((user: User, idx: number) => (
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="sm:max-w-[800px]">
          <DialogHeader><DialogTitle>{editUser ? 'Edit District' : 'Add New District'}</DialogTitle><DialogDescription>{editUser ? 'Update the details for this district.' : 'Fill in the details for the new district.'}</DialogDescription></DialogHeader>
          <form onSubmit={handleFormSubmit} className="space-y-4 mt-2 max-h-[70vh] overflow-y-auto pr-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2"><Label htmlFor="role_id">District ID*</Label><Input id="role_id" name="role_id" placeholder="e.g., DIST001" value={form.role_id[0] || ''} onChange={(e) => setForm(f => ({...f, role_id: [e.target.value]}))} required /></div>
            ...
            <DialogFooter className="pt-4">
              {editUser && (
                <Button type="button" variant={form.status === 'Active' ? 'destructive' : 'secondary'} onClick={handleStatusToggle} disabled={formLoading} className="mr-auto">
                  {form.status === 'Active' ? 'Block District' : 'Activate District'}
                </Button>
              )}
              <DialogClose asChild><Button type="button" variant="outline" disabled={formLoading}>Cancel</Button></DialogClose>
              <Button type="submit" disabled={formLoading}>{formLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin"/> : null}{formLoading ? 'Saving...' : (editUser ? 'Update District' : 'Add District')}</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
      
      <DeleteConfirmationModal open={isDeleteModalOpen} onOpenChange={setIsDeleteModalOpen} title="Delete District" description={`Are you sure you want to delete the district user "${userToDelete?.name}"? This action cannot be undone.`} onConfirm={handleConfirmDelete} isDeleting={deleteLoading} confirmButtonText="Delete District" />
    </div>
  );
}