...
// MODIFIED: Initial form state now defaults the role to 'DIV'
...
export default function DivUsers() {
  ...
  // MODIFIED: The role is now hardcoded to 'DIV' in the fetchUsers call.
  useEffect(() => {
    const params: any = {
      page: pagination.currentPage,
      role: 'DIV', // Always fetch users with the 'DIV' role
      status: status,
    };
  ...
  const openAddModal = () => {
    setEditUser(null);
    setForm(initialFormState); // This correctly sets the default role to 'DIV'
    setIsModalOpen(true);
  };
  ...
  const openEditModal = (user: User) => {
    ...
    const formState: Omit<User, '_id' | 'createdOn'> = {
      ...
      password: '',
      role: user.role ?? 'DIV', // Ensure role is set, default to DIV
  ...
  const refreshData = () => {
    // MODIFIED: Ensure refresh also fetches only 'DIV' role
    dispatch(fetchUsers({ page: pagination.currentPage, role: 'DIV', status: status, search: debouncedSearch }));
  };
  ...
  //   // Ensure the role is always 'DIV' upon submission from this page
  //   let dataToSubmit: User = { ...form, role: 'DIV' };

  //   try {
  //     if (editUser && editUser._id) {
  //       // Since this page is for DIV users, we assume no role change is intended here.
  ...
  //       if (result) {
  //         toast.success("DIV User updated successfully!");
  //         setIsModalOpen(false);
  //         refreshData();
  //       }
  //     } else {
  //       // --- ADD USER LOGIC ---
  //       const newRoleId = generateRoleId('DIV');
  //       dataToSubmit.roleId = [newRoleId];
        
  //       const result = await dispatch(addUser(dataToSubmit) as any);
  //       if (result) {
  //           toast.success("DIV User added successfully!");
  //           setIsModalOpen(false);
  //           dispatch(fetchUsers({ page: 1, role: 'DIV' }));
  //           dispatch({ type: 'users/setCurrentPage', payload: 1 });
  //       }
  //     }
  //   } catch (error: any) {
  //       toast.error(error.message || "Failed to save DIV user");
  //   } finally {
  ...
  return (
    <div className="w-full mx-auto mt-2">
      <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-8">
        ...
        <h1 className="text-4xl font-bold shrink-0">DIV User List</h1>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="sm:max-w-[800px]">
          <DialogHeader>
            <DialogTitle>
              {editUser ? 'Edit DIV User' : 'Add New DIV User'}
            </DialogTitle>
            <DialogDescription>
              {editUser ? 'Update the details for this DIV user.' : 'Fill in the details for the new DIV user.'}
            </DialogDescription>
          </DialogHeader>
          <form  className="space-y-4 mt-2 max-h-[70vh] overflow-y-auto pr-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {/* MODIFIED: Role selection is now disabled as it's fixed to 'DIV' */}
              <div className="space-y-2">
